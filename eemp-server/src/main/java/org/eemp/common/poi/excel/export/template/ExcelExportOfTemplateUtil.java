 ...
 */
package org.eemp.common.poi.excel.export.template;
...
import org.eemp.common.poi.cache.ExcelCache;
import org.eemp.common.poi.cache.ImageCache;
import org.eemp.common.poi.entity.ImageEntity;
import org.eemp.common.poi.excel.annotation.ExcelTarget;
import org.eemp.common.poi.excel.entity.TemplateExportParams;
import org.eemp.common.poi.excel.entity.enmus.ExcelType;
import org.eemp.common.poi.excel.entity.params.ExcelExportEntity;
import org.eemp.common.poi.excel.entity.params.ExcelForEachParams;
import org.eemp.common.poi.excel.entity.params.ExcelTemplateParams;
import org.eemp.common.poi.excel.export.base.ExcelExportBase;
import org.eemp.common.poi.excel.export.styler.IExcelExportStyler;
import org.eemp.common.poi.excel.html.helper.MergedRegionHelper;
import org.eemp.common.poi.exception.excel.ExcelExportException;
import org.eemp.common.poi.exception.excel.enums.ExcelExportEnum;
import org.eemp.common.poi.util.*;
	...
	private List<ExcelTemplateParams> getAllDataColumns(Cell cell, String name) {
		...
		while (true) {
			...
			if (cellStringString.contains(PoiElUtil.END_STR)) {
				...
				}else if(cellStringString.trim().equals(PoiElUtil.EMPTY)){
					//可能是合并的单元格，允许空数据的设置
					columns.add(new ExcelTemplateParams(PoiElUtil.EMPTY, tempCell.getCellStyle(), tempCell.getRow().getHeight()));
				} else {
					// 最后一行被删除了
					break;
				}
		...
		for (int k = 0; k < rowspan; k++) {
			...
			for (int i = 0; i < colspan && i < columns.size(); i++) {
				...
				if (StringUtils.isEmpty(params.getName())) {
					...
					if (isHasSymbol(tempStr, PoiElUtil.NUMBER_SYMBOL)) {
						isNumber = true;
					...
					String  dict   = null;
					if (isHasSymbol(tempStr, PoiElUtil.DICT_HANDLER)) {
						isDict = true;
						dict = tempStr.substring(tempStr.indexOf(PoiElUtil.DICT_HANDLER) + 5).split(";")[0];
						tempStr = tempStr.replaceFirst(PoiElUtil.DICT_HANDLER, "");
