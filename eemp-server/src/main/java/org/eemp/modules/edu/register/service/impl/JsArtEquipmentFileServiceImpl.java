package org.eemp.modules.edu.register.service.impl;

import org.eemp.modules.edu.register.entity.JsArtEquipmentFile;
import org.eemp.modules.edu.register.mapper.JsArtEquipmentFileMapper;
import org.eemp.modules.edu.register.service.IJsArtEquipmentFileService;
import org.springframework.stereotype.Service;

import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;

import javax.annotation.Resource;

/**
 * @Description: 初中美术教学装备配置文件
 * @Date:   2023-11-20
 * @Version: V1.0
 */
@Service
public class JsArtEquipmentFileServiceImpl extends ServiceImpl<JsArtEquipmentFileMapper, JsArtEquipmentFile> implements IJsArtEquipmentFileService {
    @Resource
    private JsArtEquipmentFileMapper mapper;

    @Override
    public int changeReported(String id, int val) {
        return mapper.changeReported(id, val);
    }

}
