...
public class EduPrimarySportEquipmentReqController extends BaseController<EduPrimarySportEquipmentReq, IEduPrimarySportEquipmentReqService> {
    ...
    public Result<IPage<EduPrimarySportEquipmentReq>> queryPageList(EduPrimarySportEquipmentReq eduPrimarySportEquipmentReq,
        ...
        String orgCode = getOrgCode(req);
        eduPrimarySportEquipmentReq.setSysOrgCode(orgCode);
        ...
        Page<EduPrimarySportEquipmentReq> page = new Page<>(pageNo, pageSize);
        ...
        if (pageList.getRecords().size() == 0) {
            ...
            pageList = eduPrimarySportEquipmentReqService.page(page, queryWrapper);
        }
...
//        eduPrimarySportEquipmentReqService.save(eduPrimarySportEquipmentReq);
     ...
     * @return
     */
//    @RequiresPermissions("edu.primarysportequipmentreq:edu_primary_sport_equipment_req:importExcel")
    ...
    // 主要用于导入Excel解析
    public List<EduPrimarySportEquipmentReq> parseExcel(InputStream inputStream, String suffix, ImportParams params, LoginUser loginUser) {

        final List<EduPrimarySportEquipmentReq> objects = new ArrayList<>();
        ...
        for (int sheetI = 0; sheetI < allSheets; sheetI++) {
            ...
            for (int rowNum = params.getHeadRows() + params.getTitleRows(); rowNum <= lastRowNum; rowNum++) {
                ...
                if (null != row) {
                    ...
                    Cell actualNumCell = row.getCell(lastCellNum - 1);
                    if (firstCell == null) {
                        continue;
                    }
                    String firstCellString = parseCell(firstCell, rowNum, 0);

                    if (StringUtils.isEmpty(firstCellString) || firstCellString.contains("分类代码") || firstCellString.contains("表") || firstCellString.contains("ID")) {
                        continue;
                    }
                    if (lastCellNum != 0) {
                        EduPrimarySportEquipmentReq req = new EduPrimarySportEquipmentReq();
                        req.setId(firstCellString);
                        req.setUpdateBy(loginUser.getUsername());
                        req.setUpdateTime(new Date());
                        req.setActualNum(Integer.parseInt(parseCell(actualNumCell, rowNum, lastCellNum - 1)));
